...
class AppDelegate: UIResponder, UIApplicationDelegate, UISplitViewControllerDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        FirebaseManager.getInstance().configure()
    ...
    func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any], fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        ...
        if state == .background || (state == .inactive && !appIsStarting) {
            ...
            DropboxManager.getInstance().downloadMagazine(name: name, progress: {total, downloaded in
            }, completion: {url in
                completionHandler(.newData)
            }, failure: {error in
                completionHandler(.failed)
            })
        } else if state == .inactive && appIsStarting {
    ...
    func registerForRemoteNotifications(_ application: UIApplication) {
        if #available(iOS 10.0, *) {
            ...
            FirebaseManager.getInstance().messagingDelegate(self)
